...
const CodeEditor: React.FC<{ code: string, onCodeChange: (newCode: string) => void }> = ({ code, onCodeChange }) => {
...
export const DevelopmentView: React.FC = () => {
    ...
    return (
        <div ref={containerRef} className="flex-1 grid grid-cols-[var(--divider-position)_auto_1fr] gap-0 p-4 sm:p-6 overflow-hidden bg-transparent h-full" style={{ '--divider-position': `${dividerPosition}%` } as React.CSSProperties}>
            ...
            <div className="flex flex-col gap-6 overflow-y-auto pr-3 -mr-3">
                <Card className="p-6">
                    ...
                    <div className="flex flex-col sm:flex-row gap-3">
                        ...
                        <div className="flex gap-3"><input type="file" multiple ref={fileInputRef} onChange={handleFileImport} className="hidden" accept=".html,.htm,.css,.js,.jsx,.ts,.tsx,.json,.md,.txt"/><button onClick={() => fileInputRef.current?.click()} disabled={isLoading} className="flex-1 w-full sm:w-auto flex items-center justify-center gap-2 px-4 py-2 bg-gray-500 text-white rounded-lg font-semibold hover:bg-gray-600 transition-colors shadow-sm focus:ring-2 focus:ring-offset-2 focus:ring-gray-400" title="Importer des fichiers locaux"><ArrowUpTrayIcon className="h-5 w-5" /><span className="hidden sm:inline">Importer</span></button>{files.length > 0 && (<button onClick={handleClearProject} disabled={isLoading} className="flex-1 w-full sm:w-auto flex items-center justify-center gap-2 px-4 py-2 bg-red-600 text-white rounded-lg font-semibold hover:bg-red-700 transition-colors shadow-sm focus:ring-2 focus:ring-offset-2 focus:ring-red-500" title="Effacer le projet actuel"><TrashIcon className="h-5 w-5" /></button>)}</div>
                    </div>
                </Card>
                ...
                {files.length > 0 && (<Card className="flex-1 flex flex-col min-h-[400px] p-0"><div className="p-2 border-b border-gray-200 dark:border-gray-700/60 flex items-center justify-between gap-2"><div className="flex items-center gap-3 shrink-0"><FolderOpenIcon className="h-6 w-6 text-gray-500 dark:text-gray-400"/> <h3 className="text-lg font-bold">Fichiers du Projet</h3></div>{activeFile && (<div className="flex items-center gap-2 shrink-0"><button onClick={() => handleRenameFile(activeFile.fileName)} className="p-1.5 rounded-md hover:bg-gray-200 dark:hover:bg-gray-700" title="Renommer"><PencilIcon className="h-4 w-4"/></button><button onClick={() => handleDeleteFile(activeFile.fileName)} className="p-1.5 rounded-md hover:bg-red-500/10 text-gray-500 dark:text-gray-400 hover:text-red-500" title="Supprimer"><TrashIcon className="h-4 w-4"/></button></div>)}</div><div className="flex items-center gap-2 p-2 bg-gray-100 dark:bg-gray-900/50 overflow-x-auto border-b border-gray-200 dark:border-gray-700/60">{files.map(file => (<button key={file.fileName} onClick={() => setActiveFileName(file.fileName)} className={`flex items-center gap-2 pl-3 pr-3 py-1.5 rounded-md text-sm font-medium transition-colors shrink-0 ${activeFileName === file.fileName ? 'bg-white dark:bg-gray-700 shadow-sm' : 'hover:bg-white/60 dark:hover:bg-gray-800/60'}`}><DocumentTextIcon className="h-5 w-5"/>{file.fileName}</button>))} <button onClick={handleNewFile} className="flex items-center gap-1.5 pl-2 pr-3 py-1.5 rounded-md text-sm font-medium transition-colors bg-gray-200 hover:bg-gray-300 dark:bg-gray-700/80 dark:hover:bg-gray-700" title="Nouveau fichier"><PlusIcon className="h-5 w-5"/>Nouveau</button></div><div className="flex-1 flex flex-col relative">{activeFile ? (<CodeEditor code={activeFile.code} onCodeChange={handleCodeChange} />) : (<div className="flex-1 flex items-center justify-center text-gray-500">Sélectionnez un fichier pour le modifier.</div>)}</div></Card>)}
            </div>
            {/* Resizer */}
            <div onMouseDown={onMouseDown} className="flex items-center justify-center cursor-col-resize group w-2.5"><div className="w-1 h-12 bg-gray-300 dark:bg-gray-600 rounded-full group-hover:bg-indigo-500 transition-colors"></div></div>
            {/* Right Column: Preview */}
            <div className="flex flex-col h-full overflow-hidden pl-3 -ml-3"><Card className="flex-1 flex flex-col p-0"><div className="p-2 border-b border-gray-200 dark:border-gray-700/60 flex items-center justify-between"><div className="flex items-center gap-1"><button onClick={() => setActivePreviewTab('preview')} className={`flex items-center gap-2 px-3 py-1.5 rounded-md text-sm font-semibold transition-colors ${activePreviewTab === 'preview' ? 'bg-indigo-100 dark:bg-indigo-900/50 text-indigo-700 dark:text-indigo-300' : 'text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-700/50'}`}><EyeIcon className="h-5 w-5"/> Aperçu</button><button onClick={() => setActivePreviewTab('console')} className={`flex items-center gap-2 px-3 py-1.5 rounded-md text-sm font-semibold transition-colors ${activePreviewTab === 'console' ? 'bg-indigo-100 dark:bg-indigo-900/50 text-indigo-700 dark:text-indigo-300' : 'text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-700/50'}`}><TerminalIcon className="h-5 w-5"/> Console {consoleMessages.length > 0 && <span className="text-xs bg-indigo-500 text-white rounded-full px-1.5 py-0.5">{consoleMessages.length}</span>}</button></div><div className="flex items-center gap-2"><button onClick={() => setDebouncedFiles([...files])} disabled={!previewUrl} className="flex items-center gap-2 px-3 py-1.5 text-sm bg-gray-200 dark:bg-gray-700 rounded-lg hover:bg-gray-300 dark:hover:bg-gray-600 transition-colors disabled:opacity-50" title="Rafraîchir"><ArrowPathIcon className="h-5 w-5"/></button><a href={previewUrl || '#'} target="_blank" rel="noopener noreferrer" className={`flex items-center gap-2 px-3 py-1.5 text-sm bg-gray-200 dark:bg-gray-700 rounded-lg hover:bg-gray-300 dark:hover:bg-gray-600 transition-colors ${!previewUrl ? 'opacity-50 pointer-events-none' : ''}`} title="Ouvrir dans un nouvel onglet"><ArrowUpRightIcon className="h-5 w-5"/></a></div></div>{missingFiles.length > 0 && (<div className="p-2 text-xs text-amber-800 dark:text-amber-300 bg-amber-500/10 flex items-center gap-2"><InformationCircleIcon className="h-4 w-4 shrink-0"/><span><strong>Attention:</strong> {missingFiles.length > 1 ? "Fichiers liés non trouvés" : "Fichier lié non trouvé"}: {missingFiles.join(', ')}</span></div>)}<div className="flex-1 bg-white dark:bg-gray-900/50 rounded-b-lg relative overflow-hidden">{(isLoading && isModifying) && (<div className="absolute inset-0 bg-black/50 backdrop-blur-sm flex flex-col items-center justify-center text-white z-20 animate-fade-in"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-white mb-2"></div>Modification en cours...</div>)}<div className={`w-full h-full ${activePreviewTab === 'preview' ? 'block' : 'hidden'}`}>{previewUrl ? (<iframe key={previewUrl} src={previewUrl} title="Project Preview" className="w-full h-full border-0" sandbox="allow-scripts allow-same-origin"/>) : (<div className="w-full h-full flex items-center justify-center text-center text-gray-500 dark:text-gray-400">{ !(isLoading && files.length === 0) && <div><CodeBracketIcon className="h-20 w-20 mx-auto mb-4 opacity-50"/><p className="font-semibold text-lg">L'aperçu de votre projet apparaîtra ici</p></div>}</div>)}</div><div className={`w-full h-full flex flex-col ${activePreviewTab === 'console' ? 'flex' : 'hidden'}`}><div className="flex-1 p-2 overflow-y-auto font-mono text-xs bg-gray-100 dark:bg-gray-900">{consoleMessages.length > 0 ? consoleMessages.map((msg, i) => (<div key={i} className={`flex gap-2 items-start p-1 border-b border-gray-200 dark:border-gray-800 ${getConsoleLineColor(msg.type)}`}><span className="text-gray-400 select-none">{msg.timestamp.toLocaleTimeString()}</span><pre className="whitespace-pre-wrap break-words">{msg.message}</pre></div>)) : <div className="p-4 text-gray-500">La console est vide. Les messages de `console.log()` apparaîtront ici.</div>}</div><div className="p-1 border-t border-gray-200 dark:border-gray-700/60 bg-gray-50 dark:bg-gray-800 flex items-center justify-between"><button onClick={() => setConsoleMessages([])} className="text-xs px-2 py-1 hover:bg-gray-200 dark:hover:bg-gray-700 rounded">Vider la console</button></div></div></div></Card></div>{error && (<div className="fixed bottom-5 left-1/2 -translate-x-1/2 z-50 p-4 max-w-md w-full bg-red-500/20 dark:bg-red-900/40 border border-red-500/30 text-red-800 dark:text-red-200 rounded-lg shadow-lg animate-slide-fade-in" role="alert"><div className="flex justify-between items-start"><div className="flex-1"><p className="font-bold">Erreur</p><p className="text-sm">{error}</p></div><button onClick={() => setError(null)} className="p-1 rounded-full hover:bg-red-500/20 -mt-1 -mr-1">&times;</button></div></div>)}</div>
    );
};